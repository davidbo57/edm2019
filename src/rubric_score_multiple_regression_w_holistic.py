...
Y = np.loadtxt("../data/dataset96.csv", dtype='float', delimiter=',', skiprows=1, usecols=(96, 97, 100, 101, 102))
...
for i in range(ITERATIONS):
    ...
    history = train_model.fit(trainX, trainY[:,0:4], epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=2)
    ...
    machine_scores[:,3] = 2*machine_scores[:,3]
    machine_scores = np.sum(machine_scores, axis=1)
    machine_scores = np.rint(machine_scores)

    human_scores = trainY[:,4]

    loss = train_model.evaluate(trainX, trainY[:,0:4], verbose=0)
    ...
    for train, test in kfold.split(trainX, trainY):
        cv_model = AES_Model()
        H = cv_model.fit(trainX[train], trainY[train,0:4], validation_data=(trainX[test], trainY[test,0:4]), epochs=EPOCHS, batch_size=BATCH_SIZE, verbose=2)
        score = cv_model.evaluate(trainX[test], trainY[test,0:4], verbose=0)
        ...
        train_machine_scores[:,3] = 2*train_machine_scores[:,3]
        train_machine_scores = np.sum(train_machine_scores, axis=1)
        ...
        test_machine_scores[:,3] = 2*test_machine_scores[:,3]
        test_machine_scores = np.sum(test_machine_scores, axis=1)
    ...
    test_score = train_model.evaluate(testX, testY[:,0:4], verbose=2)
