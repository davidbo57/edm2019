...
Y = np.loadtxt("../data/dataset96_weights.csv", dtype='float', delimiter=',', skiprows=1, usecols=(RUBRIC_6, 103))
...
for i in range(ITERATIONS):
    ...
    history = train_model.fit(trainX, trainY[:,0], epochs=EPOCHS, batch_size=BATCH_SIZE, sample_weight=trainY[:,1], verbose=2)

    machine_scores = train_model.predict(trainX, batch_size=BATCH_SIZE)
    machine_scores = np.rint(machine_scores.flatten())
    machine_scores = np.where(machine_scores > 12, 12, machine_scores)
    machine_scores = np.where(machine_scores < 2, 2, machine_scores)
    human_scores = trainY[:,0]

    loss = train_model.evaluate(trainX, trainY[:,0], sample_weight=trainY[:,1], verbose=0)
    ...
    for train, test in kfold.split(trainX, trainY):
        cv_model = AES_Model()
        H = cv_model.fit(trainX[train], trainY[train,0], validation_data=(trainX[test], trainY[test,0]), epochs=EPOCHS, batch_size=BATCH_SIZE, sample_weight=trainY[train,1], verbose=0)
        score = cv_model.evaluate(trainX[test], trainY[test,0], sample_weight=trainY[test,1], verbose=0)
        machine_scores = cv_model.predict(trainX[test], batch_size=BATCH_SIZE)
        machine_scores = np.rint(machine_scores.flatten())
        machine_scores = np.where(machine_scores > 12, 12, machine_scores)
        machine_scores = np.where(machine_scores < 2, 2, machine_scores)
        human_scores = trainY[test,0]
        ...
        train_machine_scores = train_machine_scores.flatten()
        ...
        test_machine_scores = test_machine_scores.flatten()
    ...
    test_machine_scores = np.rint(test_machine_scores.flatten())
    test_machine_scores = np.where(test_machine_scores > 12, 12, test_machine_scores)
    test_machine_scores = np.where(test_machine_scores < 2, 2, test_machine_scores)
    test_score = train_model.evaluate(testX, testY[:,0], sample_weight=testY[:,1], verbose=2)
