...
Y = np.loadtxt("../data/dataset96_weights.csv", dtype='float', delimiter=',', skiprows=1, usecols=RUBRIC_6)
W = np.loadtxt("../data/dataset96_weights.csv", dtype='float', delimiter=',', skiprows=1, usecols=103)
...
for i in range(ITERATIONS):
    ...
    history = train_model.fit(trainX, trainY[:,0:11], epochs=EPOCHS, batch_size=BATCH_SIZE, sample_weight=trainY[:,11], verbose=2)
    machine_scores = train_model.predict(trainX, batch_size=BATCH_SIZE)
    machine_scores = np.argmax(machine_scores, axis=1)+2

    human_scores = np.argmax(trainY[:,0:11], axis=1)+2

    loss = train_model.evaluate(trainX, trainY[:,0:11], sample_weight=trainY[:,11], verbose=0)
    ...
    for train, test in kfold.split(trainX, trainY):
        cv_model = AES_Model()
        H = cv_model.fit(trainX[train], trainY[train,0:11], validation_data=(trainX[test], trainY[test,0:11]), epochs=EPOCHS, batch_size=BATCH_SIZE, sample_weight=trainY[train,11], verbose=0)
        score = cv_model.evaluate(trainX[test], trainY[test,0:11], sample_weight=[trainY[test,11]], verbose=0)
        machine_scores = cv_model.predict(trainX[test], batch_size=BATCH_SIZE)
        machine_scores = np.argmax(machine_scores, axis=1)+2
        human_scores = np.argmax(trainY[test,0:11], axis=1)+2
    ...
    train_human_scores = np.argmax(trainY[:,0:11], axis=1)+2
    ...
    test_human_scores = np.argmax(testY[:,0:11], axis=1)+2
    ...
    test_score = train_model.evaluate(testX, testY[:,0:11], sample_weight=testY[:,11], verbose=2)
